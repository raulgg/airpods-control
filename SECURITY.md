...
`airpods` reaches a private Apple audio API by forging one entitlement inside its own process. That is an unusual thing to run, so this document states plainly what the mechanism does, what it does not do, and how the build-from-source model protects you.
...
- **No risk to other users or processes.** The change lives entirely inside the `airpods` process for its lifetime. No other process, user, or session is affected. Nothing is written to a shared location.
