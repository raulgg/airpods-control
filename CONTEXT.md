...
: One read-only, one-pass observation of listening mode and Conversation
  ...
  proven unsupported, unresolved, or a read error. Unsupported fields are
  absent. Unresolved fields and read errors retain the corresponding individual
  getter's fallback state, and read errors also carry a field-keyed error. A
  status command outcome contains snapshots in system audio-routing discovery
  order.
