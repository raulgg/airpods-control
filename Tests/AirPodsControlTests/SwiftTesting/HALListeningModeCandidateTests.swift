...
struct HALListeningModeCandidateTests {
  ...
  func listeningModeControlCandidatesReuseMappedOutputInventory() throws {
    ...
    #expect(candidates.first?.route == .selected, "mapped default output marks control selected")
    let transport = candidates.first?.halTransport as? HALListeningModeTransport
    #expect(transport?.audioDeviceID == 70, "control keeps the eligible playback endpoint")
    #expect(
  ...
  func listeningModeControlInventoryDefersHALStateReads() throws {
    ...
    #expect(backend.listeningModeReads.isEmpty, "control inventory does not read HAL state")
    let transport = controller.listeningModeCandidates().first?.halTransport
      as? HALListeningModeTransport
    #expect(backend.listeningModeReads.isEmpty, "candidate construction keeps HAL state lazy")
  ...
  func listeningModeAllowOffCorrelationIsLazyAndExact() throws {
    ...
    #expect(backend.deviceUIDReads == [81], "a cache-relevant selected operation reads one exact UID")
    #expect(
