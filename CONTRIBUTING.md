...
but only connected hardware can verify private-API behavior across a placement
change. Run this check on the same Mac and AirPods before merging a discovery
change.
...
2. Create an owner-only capture directory, then capture a capability baseline.
   Run all commands in the same shell and replace `My AirPods` if the
   customized name differs:
...
3. Remove one earbud without putting it in the case. Verify in System Settings
   that the AirPods remain both the selected input and selected output, then run:
...
System Settings permits in the same state. The implementation intentionally
does not add another discovery backend or make `--debug` change which devices
are discovered.
